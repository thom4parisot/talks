...
module.exports = {
  ...
  'data-background': function getDataBackgroundAttribute(content){
    var attr = function(attrContent){
      return ' data-background="'+ attrContent +'"';
    };

    if (/^(https?)?\/\//.test(content.background)){
      return attr(content.background);
    }

    if (content['background-color']) {
      return attr(content['background-color']);
    }

    if (content.background){
      return attr('images/' + content.background);
    }
  },
